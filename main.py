...
async def _coerce_reset_body(request: Request) -> ResetBody:
    """
    Parse POST body manually so FastAPI never injects a required JSON ``Body`` dependency
    (validators that send no body / null / bad Content-Type otherwise get 422 ``Field required``).
    """
